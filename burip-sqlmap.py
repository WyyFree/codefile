...
class BurpExtender(IBurpExtender, IHttpListener, IContextMenuFactory, IProxyListener, IHttpRequestResponse, IBurpExtenderCallbacks):
    ...
    def sqlmapShell(self, invocation):
        ...
        # cmdBase="tell application \"gnome-terminal\" \n\tactivate\n\tdo script \" sqlmap -r " + fullPathName + " --batch --threads 3 --tamper randomcase.py" + "\"\nend tell"
        # cmdBase="gnome-terminal -t 'sqlmap' -x bash -c \"sqlmap -r " + fullPathName + " --batch --threads 3 --tamper randomcase.py\""
        cmdBase = "gnome-terminal -x bash -c 'sqlmap -r %s --batch --threads 3 --tamper randomcase.py --level 5;exec bash'" % fullPathName

        # proc = subprocess.Popen(['osascript', '-'], shell=True, stdin=subprocess.PIPE, stdout=subprocess.PIPE)
        # stdout_output = proc.communicate(cmdBase)
        proc = subprocess.Popen(cmdBase, shell=True, stdin=subprocess.PIPE, stdout=subprocess.PIPE)
